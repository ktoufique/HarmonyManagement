...
if(repoIdSelected==15 || repoNameSelected==15){
repoIdSelected=21;
repoNameSelected="http://git.apache.org/commons-codec.git";

}
var stringCommitCompound=new Array();
var competence=new Array();
var nouvelleCompetence=new Array();
var nbString=-1;
var idConcreteUsed= new Array();
for (var i=0; i<data.aptitude_expressions.length; i++) { //Process through each expression of concrete aptitudes
		for (var j=0; j<data.aptitude_expressions[i].expressions.length; j++) { 
			...
			for(var k=0;k<data.aptitude_expressions[i].expressions[j].concrete_expressions.length;k++){
				...
				//document.write("je cherche "+repoIdSelected+" et "+idRepoRead+" convient <br>");
				...
				competence[nbString]=competence[nbString]+1;
				...
				if(nouvelleCompetence[nbString]==1){
					var tempIndex=idConcreteAptitudes.indexOf(data.aptitude_expressions[i].expressions[j].id_concrete_aptitude);
					if(tempIndex!=-1){
					var tempName= concreteAptitudes[tempIndex].desc;
					stringCommitCompound[nbString]=stringCommitCompound[nbString]+","+tempName;
					}
					nouvelleCompetence[nbString]=0;
...
//aaa
  ...
  for(var j=0;j<nbData;++j){
    ...
    console.timeEnd("LineChartProjet");
